document.addEventListener('DOMContentLoaded', async function () {
    ...
    inputNome.addEventListener('input', async () => {
        const response = await fetch(`${baseUrl}/api/estoque/produtos?nome=${inputNome.value}`, {
            headers: headers
        });
    ...
    function preencherTabela(produtos) {
        ...
        produtos.forEach(produto => {
            const dataFormatada = new Date(produto.data_entrada).toLocaleDateString('pt-BR'); // Formata a data para o formato brasileiro
            ...
            const estoqueClass = produto.qntd_produto <= 10 && produto.status_produto !== 'Indisponível' ? 'estoque-baixo' : '';
            const statusClass = produto.status_produto == 'Indisponível' && produto.qntd_produto === 0 ? 'indisponivel' : '';
    ...
    document.querySelector('#submitform').addEventListener('click', async () => {
        ...
        const categoriaOption = document.querySelector(`#categorias-list option[value='${categoriaInput.value}']`);

        // Criar objeto com os dados
        const dados = {
            nome: document.querySelector('#nome-produto').value,
            preco_produto: parseFloat(document.querySelector('#preco-produto').dataset.valor),
            descricao: document.querySelector('#complemento-produto').value,
            id_participante: parseInt(document.querySelector('#codigo-participante-produto').dataset.id),
    ...
    const inputFornecedor = document.querySelector('#codigo-participante-produto');
    if (inputFornecedor) {
    ...
    document.querySelector('#closeModalFornecedor').addEventListener('click', () => {
        document.querySelector('#modalFornecedor').close();
    });

    // Evento para salvar fornecedor
    document.querySelector('#salvarFornecedor').addEventListener('click', async () => {
        ...
        const dados = {
            ...
            telefone_participante: document.querySelector('#telefone-fornecedor').value,
        ...
        try {
            ...
            document.querySelector('#codigo-participante-produto').value = dados.nome_participante;
            document.querySelector('#codigo-participante-produto').dataset.id = novoFornecedor.id_participante;
    ...
    document.querySelector('#closeModalCategoria').addEventListener('click', () => {
        document.querySelector('#modalCategoria').close();
    });

    document.querySelector('#salvarCategoria').addEventListener('click', async () => {
        const dados = {
        ...
        try {
            ...
            if (response.ok) {
                ...
                document.querySelector('#categoria-produto').value = dados.nome_categoria;
                document.querySelector('#categoria-produto').dataset.id = novaCategoria.id_categoria;
