document.addEventListener("DOMContentLoaded", async function () {
  ...
  async function carregarDados() {
    try {
      ...
      if (!responseParticipantes.ok) {
        throw new Error(
          `Erro ao carregar participantes: ${responseParticipantes.status}`
        );
      }
      ...
      if (!responseFuncionarios.ok) {
        throw new Error(
          `Erro ao carregar funcionários: ${responseFuncionarios.status}`
        );
      }
      ...
      setupAutocomplete(
        document.getElementById("participante_nome"),
        participantes,
        "id_participante",
        "nome_participante"
      );
      ...
      setupAutocomplete(
        document.getElementById("funcionario_nome"),
        funcionarios,
        "id_funcionario",
        "nome_funcionario"
      );
      ...
      document
        .querySelectorAll('input[name="id_tipo_movimentacao"]')
        .forEach((radio) => {
          radio.addEventListener("change", handleTipoMovimentacaoChange);
        });

      // Verificar o tipo de movimentação inicial
      const tipoMovimentacaoSelecionado = document.querySelector(
        'input[name="id_tipo_movimentacao"]:checked'
      );
      ...
      if (error.message.includes("401")) {
        ...
        alert(
          "Erro ao carregar dados iniciais. Por favor, recarregue a página."
        );
      }
    }
  }
            ...
            if (this.dataset.preco) {
              ...
              if (row) {
                ...
                if (valorUnitarioInput) {
                  valorUnitarioInput.value = parseFloat(
                    this.dataset.preco
                  ).toFixed(2);
  ...
  function updateSubtotal() {
    ...
    const valorUnitario =
      parseFloat(row.querySelector(".valor-unitario").value) || 0;
    ...
    .addEventListener("submit", async function (e) {
      ...
      document.querySelectorAll(".item-row").forEach((row) => {
        ...
        if (isNaN(valorUnitario) || valorUnitario <= 0) {
          alert(
            "O valor unitário deve ser maior que zero para todos os itens."
          );
          invalid = true;
          return;
        }
      ...
      const formData = {
        id_tipo_movimentacao: document.querySelector(
          'input[name="id_tipo_movimentacao"]:checked'
        ).value,
      ...
      try {
        ...
        if (!response.ok) {
          ...
          throw new Error(
            errorData.message || "Erro ao registrar movimentação"
          );
        }
