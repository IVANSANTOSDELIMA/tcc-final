document.addEventListener("DOMContentLoaded", async function () {
  ...
  inputNome.addEventListener("input", async () => {
    const response = await fetch(
      `${baseUrl}/api/funcionarios?nome=${inputNome.value}`,
      {
        headers: headers,
      }
    );
  ...
  document.querySelector("#filtro-cargo").addEventListener("change", (e) => {
    ...
    const funcionariosFiltrados = funcionariosGlobal.filter(
      (f) => !cargoSelecionado || f.cargo === cargoSelecionado
    );
  ...
  window.editarFuncionario = async (id) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        document.querySelector("#editar-nivelAcesso").value =
        ...
        document.querySelector("#editar-dataAdmissao").value = new Date(
          funcionario.data_admissao
        )
          .toISOString()
          .split("T")[0];
        document.querySelector("#editar-telefone").value =
        ...
        document.querySelector("#editar-complemento").value =
  ...
  window.visualizarFuncionario = async (id) => {
    try {
      ...
      if (response.ok) {
        ...
        // Preenche os campos
        document.querySelector("#visualizar-nome").value =
          funcionario.nome_funcionario;
        document.querySelector("#visualizar-nivelAcesso").value =
          funcionario.nivel_acesso;
        ...
        document.querySelector("#visualizar-dataAdmissao").value =
          dataFormatada;
        document.querySelector("#visualizar-telefone").value =
          funcionario.telefone_funcionario;
  ...
  if (salvarEdicao) {
    salvarEdicao.addEventListener("click", async () => {
      ...
      try {
        const response = await fetch(
          `${baseUrl}/api/funcionarios/${funcionarioIdParaEditar}`,
          {
            method: "PUT",
            headers: {
              Authorization: `Bearer ${token}`,
        ...
        if (response.ok) {
          ...
          if (fileInput.files[0]) {
            ...
            await fetch(
              `${baseUrl}/api/funcionarios/${funcionarioIdParaEditar}/imagem`,
              {
                method: "PUT",
                headers: {
                  Authorization: `Bearer ${token}`,
