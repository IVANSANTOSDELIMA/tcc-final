document.addEventListener("DOMContentLoaded", async function () {
  ...
  try {
    ...
    const totalEstoque = produtos.reduce(
      (total, produto) => total + produto.qntd_produto,
      0
    );
    ...
    ).length; // Filtra produtos que são -/= que 10 e não estão indisponíveis
    document.querySelector("#B_Estoque strong").textContent =
    ...
    const entradas = movimentacoes.filter(
      (mov) => mov.id_tipo_movimentacao === "E"
    ).length; // Filtra as movimentações de entrada
    const saidas = movimentacoes.filter(
      (mov) => mov.id_tipo_movimentacao === "S"
    ).length; // Filtra as movimentações de saída
  ...
  const responseMovimentacoes = await fetch(
    `${baseUrl}/api/estoque/movimentacao`,
    {
      headers: {
        Authorization: `Bearer ${localStorage.getItem("token")}`,
      },
